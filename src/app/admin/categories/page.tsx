    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500">Organize your courses by categories for better discoverability.</p>
        </div>
        <Button
          variant="primary"
          onClick={() => {
      ...
      {/* Add/Edit Form */}
      {(isAdding || isEditing) && (
        <div className="bg-white p-6 rounded-xl soft-shadow border border-slate-100 animate-in slide-in-from-top duration-300">
      ...
      <div className="bg-white rounded-xl soft-shadow border border-slate-100 overflow-hidden">
        <table className="w-full text-left border-collapse">
          <thead className="bg-slate-50 border-b border-slate-100">
            <tr>
              <th className="p-4 text-sm font-bold text-slate-700">Category Name</th>
              <th className="p-4 text-sm font-bold text-slate-700">Slug</th>
          ...
          <tbody>
            {categories.map((cat) => (
              <tr key={cat.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                <td className="p-4 text-sm text-slate-900 font-medium">{cat.name}</td>
