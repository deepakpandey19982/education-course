...
export async function POST(req: Request) {
  try {
    ...
    // Resolve File Buffer: Check in-memory cache first if fileId provided
    ...
    if (!buffer) {
      if (!file) {
        return new Response(
          JSON.stringify({ error: 'No source file provided or session expired. Please upload the file.' }),
          {
            status: 400,
            headers: { 'Content-Type': 'application/json' },
          }
    ...
    // Fetch existing series and subjects for destination dropdown and matching
        ...
        try {
          ...
          sendEvent({
            ...
            data: {
              ...
              existing_series_list: allSeries || [],
              existing_tests_list: allTests || [],
          ...
          console.error('Error during streaming parse in Question Formatter:', err);
          sendEvent({
            type: 'error',
            error: err.message || 'An error occurred while parsing the question file.',
          });
          controller.close();
        }
      },
    ...
    console.error('API /api/admin/question-formatter/parse-stream error:', error);
    return new Response(
      JSON.stringify({
        success: false,
